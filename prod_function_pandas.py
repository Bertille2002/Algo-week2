...
def delete_product():
  ...
  try:  # Utilisation de try en cas d'erreurs du fichier
    df = pd.read_csv(produits)  # Changed from 'products_functions.py' to 'products.csv'
    reader = pd.DataFrame(open(produits))  # Added open function to read the csv
...
def binary_search(chemin) :
    ...
    result = df[df['P_name'] == product_name]
    if result = True :
        print("Product found :")
        print(result)
    else :
